...
import { LogicComponentSchema as LogicComponentOptions } from './logic-component.schema';

export default function(options: LogicComponentOptions): Rule {
  fixOptions(options);
  ...
  return (host: Tree, context: SchematicContext) => {
    const templatesSrc: Source = apply(
      ...
      [
        template(options),
        move(`${options.appPath}/${options.logicComponentsPath}`)
      ]
    );
